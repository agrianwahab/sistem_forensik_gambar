...
from tasks import process_analysis_step_task
...
@analysis_bp.route('/process/<int:analysis_id>/step/<int:step_id>', methods=['GET', 'POST'])
def process_step_page(analysis_id, step_id):
    ...
    task_id_for_template = None

    if request.method == 'POST':
        params_for_task = {}
        if current_step_info.get('params_needed'):
...
def serve_analysis_result(analysis_folder, filename):
    safe_analysis_folder = secure_filename(analysis_folder)
    safe_filename = secure_filename(filename)

    results_dir = os.path.join(current_app.config['RESULTS_BASE_DIR'], safe_analysis_folder)
...
def handle_join_room_event(data):
    ...
    if analysis_id:
        ...
        socketio.join_room(room_name, sid=request.sid)
